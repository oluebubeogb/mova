<?php
/**
 * Lengthy theme layout — minimal, fast, blue text-links
 * No scroll animation JS. System fonts. Self-contained CSS.
 */
...
<head>
    ...
    <!-- Font Awesome: non-blocking load + font-display swap so icons never stay blank on first visit -->
    <link rel="preload" as="style" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.2/css/all.min.css" integrity="sha512-SnH5WK+bZxgPHs44uWIX+LLJAJ9/2PkPKZ5QiAj6Ta86w+fsb2TkcmfRyVX3pBnMFcV7oQPJkl9QevSCWr3W6A==" crossorigin="anonymous" onload="this.onload=null;this.rel='stylesheet'">
    ...
    <?php
    // Self-contained CSS — no extra /assets file required
    $lyCss = (isset($themePath) ? $themePath : __DIR__) . '/lengthy.css';
    if (!is_file($lyCss) && is_file(__DIR__ . '/lengthy.css')) {
        $lyCss = __DIR__ . '/lengthy.css';
    }
    if (is_file($lyCss)) {
        echo "<style id=\"lengthy-theme\">\n" . file_get_contents($lyCss) . "\n</style>\n";
    }
    ?>
    <?php
